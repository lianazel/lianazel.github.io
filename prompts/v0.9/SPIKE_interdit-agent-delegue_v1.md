# SPIKE — Un interdit d'outil suit-il un agent délégué ? · v1
...
> **Sujet neuf.** Compteur de verdicts non-`SHIP` sur le nom `interdit-agent-delegue` : **0**.
...
| P7 | Ce second fichier n'autorise **pas** les deux outils visés | `grep -cE 'WebFetch\|WebSearch' .claude/settings.local.json` | `0` **ligne** | **ARRÊTE-TOI** : le contrat n'est plus celui qu'on croit mesurer |

> **Pourquoi P5 à P7 existent.** Le contrat de permissions n'est **pas un fichier, il en est deux** :
...
| # | Ce qui doit être vrai | Commande | Attendu | Sinon |
|---|---|---|---|---|
| P8 | Le dépôt déclare **trois** types d'agents, et on les compte **fichier par fichier** | `grep -c '^tools:' .claude/agents/diagnostician.md` puis `.claude/agents/prompt-reviewer.md` puis `.claude/agents/reviewer.md` | trois comptes de **lignes**, à relever tels quels | note l'écart au rapport, **ne t'arrête pas** |
...
1. Les huit prérequis P0 à P7 sont mesurés et rapportés avec leur valeur observée. Un écart sur P1 à P7
   arrête l'essai.
...
1. **Enregistrer ce prompt**, seul, avec le message exact :
   `docs(prompt): prompts/v0.9/SPIKE_interdit-agent-delegue_v1.md`
   Ce geste se fait **avant** l'essai, pour que la session d'essai soit réellement neuve.
...
*Prompt rédigé par Cowork le 13 septembre 2026 · un seul exemplaire, dépôt qui fait foi :
