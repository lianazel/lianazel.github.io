...
**Statut** : PÉRIMÉ — remplacé avant exécution par `EVOL_menu-debordement_v2.md` (déclaration fausse sur `VISION_METHOD`)
...
| `VISION_METHOD` | — | **Introuvable** | Prescrit par la table §8.1 du référentiel, le fichier n'existe pas. La validation visuelle reste **entièrement humaine** (dette D-1, arbitrée le 9 août : validation manuelle **outillée**, protocole écrit au référentiel). |
...
Un **contrôle bloquant supplémentaire** dans `scripts/check-i18n.mjs` : la largeur plancher de la
barre de navigation — somme des libellés **de la langue la plus longue** plus les espacements
déclarés — ne dépasse pas la place disponible à la largeur d'écran minimale visée.

Quatre exigences, aucune facultative :

1. **Les deux langues, et on retient la pire.** C'est précisément ce que le modèle n'avait pas su
   faire le 9 août : il avait calculé une seule fois, et le défaut n'existait qu'en français.
